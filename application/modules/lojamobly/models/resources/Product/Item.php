<?php
/**
 * LOJAMOBLY_Resource_Product_Item
 * 
 * @category   Storefront
 ...
 */
class LOJAMOBLY_Resource_Product_Item extends LOJAMOBLY_Model_Resource_Db_Table_Row_Abstract implements LOJAMOBLY_Resource_Product_Item_Interface
{   
    /**
     * Get product images
     *
     * @param  boolean $includeDefault Whether to include the default
     * @return array Containing LOJAMOBLY_Resource_ProductImage_Item
     */
    public function getImages($includeDefault=false)
    {
        ...
        return $this->findDependentRowset('LOJAMOBLY_Resource_Productimage',
            'Image', 
            $select
        );
    }
     ...
     *
     * @return LOJAMOBLY_Resource_ProductImage_Item
     */
    public function getDefaultImage()
    {
        $row = $this->findDependentRowset('LOJAMOBLY_Resource_Productimage',
            'Image', 
            $this->select()
                 ->where('isDefault = ?', 'Yes')
                 ->limit(1)
    ...
    public function getPrice($withDiscount=true,$withTax=true)
    {
        ...
        if (true === $this->isTaxable() && true === $withTax) {
            $taxService = new LOJAMOBLY_Service_Taxation();
