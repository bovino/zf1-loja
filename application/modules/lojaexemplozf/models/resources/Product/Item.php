<?php
/**
 * LOJAEXEMPLOZF_Resource_Product_Item
 * 
 * @category   Storefront
 ...
 */
class LOJAEXEMPLOZF_Resource_Product_Item extends LOJAEXEMPLOZF_Model_Resource_Db_Table_Row_Abstract implements LOJAEXEMPLOZF_Resource_Product_Item_Interface
{   
    /**
     * Get product images
     *
     * @param  boolean $includeDefault Whether to include the default
     * @return array Containing LOJAEXEMPLOZF_Resource_ProductImage_Item
     */
    public function getImages($includeDefault=false)
    {
        ...
        return $this->findDependentRowset('LOJAEXEMPLOZF_Resource_Productimage',
            'Image', 
            $select
        );
    }
     ...
     *
     * @return LOJAEXEMPLOZF_Resource_ProductImage_Item
     */
    public function getDefaultImage()
    {
        $row = $this->findDependentRowset('LOJAEXEMPLOZF_Resource_Productimage',
            'Image', 
            $this->select()
                 ->where('isDefault = ?', 'Yes')
                 ->limit(1)
    ...
    public function getPrice($withDiscount=true,$withTax=true)
    {
        ...
        if (true === $this->isTaxable() && true === $withTax) {
            $taxService = new LOJAEXEMPLOZF_Service_Taxation();
